...
def load_reduce_convert(image_file, reduce):
    ...
    if channels == 3:
        lab_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2HSV)
        # Split LAB channels
...
def find(image, templates, debug):
    ...
    for template in templates:
        ...
        tl = min_loc
        br = (tl[0] + w, tl[1] + h)
        positions.append([min_loc, tl, br])
...
def main():
    ...
    parser.add_argument('--reduction', '-r', type=int, default=1)
    ...
    for template in args.templates:
        H, s, v = load_reduce_convert(template, args.reduction)
        templates.append(H)



    H, s, v = load_reduce_convert(args.image, args.reduction)
    if len(templates) > 0:
        find(H, templates, True)
