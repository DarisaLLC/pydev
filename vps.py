...
def rectify(img, parallel_A, parallel_B, H_A=None, debug=False):
    ...
    if debug is True:
        ...
        for line in new_parallel_A:
            ...
            cv2.line(new_img, (line[0], line[1]), (int(new_vanishing_point_A[0]), int(new_vanishing_point_A[1])),
                     (255, 0, 0), 3)

        for line in new_parallel_B:
            cv2.line(new_img, (line[0], line[1]), (line[2], line[3]), (0, 0, 255), 3)
            cv2.line(new_img, (line[0], line[1]), (int(new_vanishing_point_B[0]), int(new_vanishing_point_B[1])),
                     (0, 0, 255), 3)
